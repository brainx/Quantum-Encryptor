...
st.markdown(f"""
Utilizes **{cfg.KEM_ALG}** + **AES-256-GCM** hybrid encryption.
...
if 'password_gen' not in st.session_state: st.session_state.password_gen = ""
if 'password_gen_confirm' not in st.session_state: st.session_state.password_gen_confirm = ""
if 'password_decrypt' not in st.session_state: st.session_state.password_decrypt = ""

# === Key Generation ===
if operation == "Generate Keys":
    st.header("🔑 Generate PQC Key Pair")
    st.markdown(f"Generates a **{cfg.KEM_ALG}** public/private key pair (PEM format).")
    st.info(f"Using KEM Algorithm: **{cfg.KEM_ALG}**")
    ...
    st.markdown("---")
    if st.button(f"Generate {cfg.KEM_ALG} Key Pair", key="gen_button", disabled=disable_gen_button):
        final_password = st.session_state.password_gen if use_password and password_valid else None

        with st.status(f"Generating {cfg.KEM_ALG} keys...", expanded=True) as status:
            st.write("Generating raw OQS key pair...")
            raw_pub_key, raw_priv_key = core.generate_oqs_keys(cfg.KEM_ALG)

            if raw_pub_key and raw_priv_key:
                st.write("Raw keys generated.")
                st.write("Formatting Public Key (PEM)...")
                pub_pem = core.save_key_pem(raw_pub_key, cfg.KEM_ALG, "public")

                st.write("Formatting Private Key (PEM)...")
                if final_password:
                     st.write("(Encrypting with password...)")
                priv_pem = core.save_key_pem(raw_priv_key, cfg.KEM_ALG, "private", password=final_password)
                ...
                if pub_pem and priv_pem:
                    ...
                    st.warning("🚨 **CRITICAL:** Securely store the **Private Key** file. If password protected, remember the password! Loss = permanent data loss.")

                    pub_filename = f"{cfg.KEM_ALG.lower()}_public.pem"
                    priv_filename = f"{cfg.KEM_ALG.lower()}_private.pem"
                ...
                err_msg = f"Failed to generate raw keys for {cfg.KEM_ALG}."
                logger.error(err_msg)
                status.update(label=err_msg, state="error")
...
# === Encryption ===
    ...
    public_key_pem_file = st.file_uploader("2. Upload Recipient's Public Key (.pem)", type=['pem'], key="enc_pubkey_input")
    ...
    if uploaded_file and public_key_pem_file:
        ...
        try:
            ...
            st.error(f"Error reading public key file: {e}")
        ...
        if pub_pem_content:
            ...
            if pub_key_bytes and key_type == "public":
                ...
                output_filename = st.text_input("3. Encrypted file name:", value=suggested_output_filename, key="enc_output_name")

                if not output_filename:
                     st.warning("Please provide a name for the encrypted file.")
                elif st.button("Encrypt File", key="enc_button"):
                    input_data = uploaded_file.getvalue()
                    if len(input_data) == 0:
                         st.warning("Input file is empty. Encryption will proceed, resulting file will contain header and metadata only.")
                    elif len(input_data) > 100 * 1024 * 1024: # Warn for large files (>100MB)
                         st.warning("Input file is large. Encryption may consume significant memory and time.")
                    ...
                    with st.status(f"Encrypting '{uploaded_file.name}'...", expanded=False) as status:
                        try:
                            ...
                            if encrypted_blob:
                                ...
                                display_pem_download(encrypted_blob.decode('latin-1'), output_filename, "Encrypted File") # Use latin-1 for bytes in download helper? No, download_button takes bytes.
                                st.download_button(
                            ...
                            status.update(label=f"Encryption Error: {e}", state="error")
                ...
                st.error("Failed to load or validate the Public Key from the provided PEM file. Is it correctly formatted?")
...
# === Decryption ===
    ...
    if encrypted_file and private_key_pem_file:
        ...
        try:
            ...
            st.error(f"Error reading private key file: {e}")
        ...
        if priv_pem_content:
            if priv_type == "Private" and priv_encrypted:
                ...
                st.session_state.password_decrypt = st.text_input("Enter Private Key Password:", type="password", key="dec_pw")
                if st.session_state.password_decrypt:
                 ...
                 st.error("Error: The uploaded key file appears to be a Public Key. A Private Key is required for decryption.")
                 priv_pem_content = None # Halt
            elif not priv_type:
        ...
        if priv_pem_content and (not password_needed or password_provided):
            ...
            output_filename = st.text_input("3. Decrypted file name:", value=suggested_output_filename, key="dec_output_name")

            disable_dec_button = not output_filename or (password_needed and not password_provided)
            ...
            if st.button("Decrypt File", key="dec_button", disabled=disable_dec_button):
                ...
                if len(encrypted_blob) == 0:
                    ...
                    if len(encrypted_blob) > 100 * 1024 * 1024: # Warn for large files (>100MB)
                         st.warning("Encrypted file is large. Decryption may consume significant memory and time.")
                    ...
                    with st.status(f"Decrypting '{encrypted_file.name}'...", expanded=False) as status:
                        try:
                            ...
                            if not priv_key_bytes or key_type != "private":
                                ...
                                decrypted_data = core.decrypt_file_pro(encrypted_blob, priv_key_bytes)
                                ...
                                if decrypted_data is not None:
                                    ...
                                    logger.error(err_msg + " (Core function returned None - check for auth errors in logs)")
                                    status.update(label=err_msg, state="error")
                                    st.error(err_msg + " Possible reasons: incorrect private key, wrong password, file corrupted/tampered with.")

                        except Exception as e:
                            logger.exception(f"Unhandled exception during decryption action: {e}")
                            status.update(label=f"Decryption Error: {e}", state="error")
...
# === Key Utilities ===
    ...
    if key_util_file:
        try:
            ...
            st.error(f"Could not read or parse the key file: {e}")